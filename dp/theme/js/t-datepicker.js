...
;(function($){
    ...
    $.fn.tDatePicker = function( pr_el, options ) {
        ...
        if(settings.startOfWeek!=0&&runOnce){
            for(var j=0;j<settings.startOfWeek;j++){

                //settings.titleDays.push(settings.titleDays.shift());
                settings.titleDays.unshift(settings.titleDays.pop())
        ...
        // Admission Elements [dates], date_utc = [1,2]
            ...
            if ( numCalendar >= 0  ) {
                for ( var i_num = 0; i_num <= numCalendar; i_num++) {
                    ...
                    while ( Date.UTC(date.getFullYear(), (date.getMonth()) ) === nextDate ) {
                        DummyDay=date.getDay();
                        if(update_options.startOfWeek!=0){
                            DummyDay=DummyDay+update_options.startOfWeek;
                            if(DummyDay >= 7){
                               DummyDay=DummyDay-7;
                            }
                            //  DummyDay=(DummyDay+update_options.startOfWeek)==-1?6:(DummyDay-update_options.startOfWeek);
                        }
                        days.push(DummyDay);      // Day of week 0 - 6 Find the first and last day of the month
                        dataDays.push(date.getDate()); // Day of month 1 -31 find the number of days of a month
                        dataUTCDate.push(Date.UTC(date.getFullYear(), date.getMonth(), date.getDate())) // Number day ex: 1519257600000
                        date.setDate(date.getDate() + 1); // Compare the number of days of the month
                    }
                     console.log(days)
                     console.log(dataDays)
                     console.log(dataUTCDate)
